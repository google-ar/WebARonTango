...
VRSeeThroughCamera* VRDisplay::getSeeThroughCamera()
{
    ...
    {
        device::blink::VRSeeThroughCameraPtr seeThroughCameraPtr = controller()->getSeeThroughCamera(m_displayId);
        m_seeThroughCamera->setSeeThroughCamera(seeThroughCameraPtr);
    }
...
{
    mojo::WTFArray<float> poseMatrix = controller()->getPoseMatrix(m_displayId);
